...
import (
	...
	"github.com/vendasta/mscli/pkg/dns/plan"
...
func (p *GoogleProvider) submitChange(change *dns.Change) error {
	if len(change.Additions) == 0 && len(change.Deletions) == 0 {
		log.Printf("All records are already up to date")
		return nil
	}

	for _, del := range change.Deletions {
		log.Printf("Del records: %s %s %s", del.Name, del.Type, del.Rrdatas)
	}
	for _, add := range change.Additions {
		log.Printf("Add records: %s %s %s", add.Name, add.Type, add.Rrdatas)
	}
